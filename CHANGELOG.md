...
- **Calibration**: 2 functions — whether a model's stated confidence matches how often it is right
  ...
  **An ECE of zero does not mean calibrated** — it means the confidences and outcomes averaged out within each bin of that partition, and errors in opposite directions inside one bin cancel. A model that says 0.4 and is always right, and 0.6 and is always wrong, scores 0 at two bins and 0.55 at ten. The bin edges are left-closed `[m/M, (m+1)/M)` with `1.0` in the last bin; Guo et al. define them right-closed, which differs only for a confidence landing exactly on an edge.

  The two are worth reading together. Brier is a proper scoring rule and ECE is not, so it cannot be zeroed that way (the model above scores 0.36, worse than a coin). The other direction is a model predicting the base rate for everything: perfectly calibrated (ECE 0) and useless (Brier 0.25 at a 50% base rate).
